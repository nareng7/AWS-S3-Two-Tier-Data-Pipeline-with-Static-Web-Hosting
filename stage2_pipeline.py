...
stage2_pipeline.py
────────────────────────────────────────────────────────────
AWS S3 pipeline (Stage 2: Scheduler)

CHANGE FROM STAGE 1:
  Stage 1 hardcoded CSV_KEY = "t20.csv". That only works for
  manual runs where YOU know the exact filename.

  EventBridge Scheduler fires on a timer — it has no idea
  which file is the latest, it just runs the script "blind."
  So this version adds get_csv_key(), which:
    - uses a filename if one is passed as a command-line
      argument (this is what Stage 3's Lambda will do later)
    - otherwise lists Bucket 1 and auto-picks the most
      recently uploaded CSV (this is what Stage 2 needs)

  This one change is enough to support BOTH stage 2 and
  stage 3 — no further pipeline changes needed after this.

FLOW:
  S3 Bucket 1 (private vault)
      ↓  boto3 finds the right CSV and reads it into RAM
  EC2 + Python
...
RUN MANUALLY:
  python3 stage2_pipeline.py                  → auto-detects latest CSV
  python3 stage2_pipeline.py "some_file.csv"   → uses that exact file

TRIGGERED BY EVENTBRIDGE SCHEDULER (Stage 2):
  AWS Systems Manager runs:
  cd /home/ec2-user && python3 stage2_pipeline.py
  (no filename passed -> auto-detects the latest CSV in the bucket)
"""
...
# NOTE: there is no hardcoded CSV_KEY anymore.
# The filename is now resolved dynamically by get_csv_key() below,
# because we cannot guarantee the uploaded file will always be
# named the same thing.


# ════════════════════════════════════════════════════════════
#  SECTION 1b — DYNAMIC FILE RESOLUTION
#  This is the new piece added in Stage 2.
#
#  get_latest_file()
#    Lists every object in Bucket 1 and returns the one with
#    the most recent LastModified timestamp — S3 tracks this
#    automatically for every file, so no naming convention or
#    date-in-filename trick is needed.
#
#  get_csv_key()
#    Checks if a filename was passed in as a command-line
#    argument (sys.argv). If yes, use it exactly — this is
#    how Stage 3's Lambda will hand over the precise file
#    from the S3 event payload. If no argument was given
#    (Stage 2's scheduled run, or a plain manual run), fall
#    back to auto-detecting the latest file.
# ════════════════════════════════════════════════════════════

def get_latest_file(bucket):
    print(f"  No filename given — checking s3://{bucket} for the latest file ...")
    s3 = boto3.client("s3")
    response = s3.list_objects_v2(Bucket=bucket)

    if "Contents" not in response:
        raise Exception(f"No files found in s3://{bucket}")

    csv_files = [obj for obj in response["Contents"] if obj["Key"].endswith(".csv")]
    if not csv_files:
        raise Exception(f"No CSV files found in s3://{bucket}")

    # max() with a key function finds the object with the latest LastModified
    latest = max(csv_files, key=lambda obj: obj["LastModified"])
    print(f"  Auto-detected: {latest['Key']}  (uploaded {latest['LastModified']})")
    return latest["Key"]
...
def get_csv_key():
    if len(sys.argv) > 1:
        key = sys.argv[1]
        print(f"  Filename passed in directly: {key}")
        return key
    return get_latest_file(VAULT_BUCKET)
...
<body>
  ...
  <footer>
    Generated by <span>stage2_pipeline.py</span>
